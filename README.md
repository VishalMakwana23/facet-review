...
`facet-review@0.1.0-alpha.1` is publicly available on npm. The CLI and local release path are automated and tested across Windows, macOS, and Ubuntu on supported Node versions. This is still an alpha: hands-on screen-reader acceptance, broader human preference testing, cross-version rollback, and universal plugin-directory publication remain open release work.
